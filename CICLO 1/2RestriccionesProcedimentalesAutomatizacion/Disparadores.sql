...
FOR EACH ROW
BEGIN 
    IF :NEW.FECHA <> :OLD.FECHA OR :NEW.estudiante <> :OLD.estudiante THEN
...
delete from calificaciones where idTarea = 4;
INSERT INTO calificaciones(idTarea,puntaje,comentario,estudiante) VALUES (110,45,'Excelente trabajo',1002546);

----Ejecutando TR_TAREA_CALIFICACION_BD----OK
INSERT INTO calificaciones(idTarea,puntaje,fecha,comentario,estudiante)
VALUES(4,30,TO_DATE('2024-04-25', 'YYYY-MM-DD'),'Falta punto 4',1002546);
delete from TAREAS where idTarea = 4;
--Falla debido a que no se pueden elimina una tarea con una calificacion asignada
...
UPDATE ClientesPremium SET membresia = 'PE' WHERE USUARIOE = 1077083;
UPDATE ClientesPremium SET telefono = 300681785 WHERE USUARIOE = 1005896;

---Ejecutando TR_ESTUDIANTEMatricula_AI----
SELECT * FROM CURSOS;
select * from ESTUDIANTES;
SELECT * FROM ESTUDIANTESCURSOS;
INSERT INTO ESTUDIANTESCURSOS VALUES(1002546,101);
DELETE FROM ESTUDIANTESCURSOS WHERE USUARIO = 1002546;
